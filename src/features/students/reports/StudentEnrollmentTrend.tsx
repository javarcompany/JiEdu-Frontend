...
import StudentPredictionModal from "./StudentPredictionModal";
...
export default function StudentEnrollmentTrendChart() {
    const [showPrediction, setShowPrediction] = useState(false);
    const token = localStorage.getItem("access");
    const [data, setData] = useState<EnrollmentTrendData[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await fetch("/api/institution-enrollment-trend/",
    ...
    const studentSeries = data.map((item) => item.students);
    ...
    return (
        <div className="bg-white p-4 rounded-xl shadow-md mb-4 dark:bg-white/[0.05]">
            <div className="flex justify-between items-center mb-2">
                ...
                <StudentPredictionModal open={showPrediction} onClose={() => setShowPrediction(false)} />
            </div>
